...
2018.11.11 V1.0
    创建工程，实现jacefs文件系统。
    1）增加文件创建接口，日前版本仅用一个页来记录文件信息，最大文件数 JACEFS_FILE_NUM 定义为一页可存文件数
...
typedef struct{
    ...
    /* 文件标识 */
    uint16_t is_continues:1;/* 文件是否连续存储 */
...
//{
//    /*
//     * 为了节省内存，现在把定时器栈改小了，所以不要在这里直接操作flash，以防栈溢出
//     */
//    //jacefs_sync();
//
//    sys_event_t evt;
...
-offset     开始寻找的文件偏移数（这里指的是在文件描述区中的索引下标），返回找到时文件偏移
...
@brief : 获取文件对应的真实物理地址
                         ...
                         文件存储 连续的情况下，返回正确的物理地址，不连续的情况下返回 0xffffffff
...
//	GLOBAL_INT_DISABLE();
	...
	OS_LOG("---->FS read\r\n");
	{
        ...
        //FS 非连续读取100KB，每次读128B，耗时650ms --优化2(用RAM缓存)：耗时33ms
        len=0;
        tick1=os_get_tick();
    ...
    OS_LOG("---->MEMORY_QSPIF_S_BASE direct read\r\n"); //现在测试出用这种方法速度最快，所以读取flash时不用SDK的nvms，用直接读取
    {
...
//    GLOBAL_INT_RESTORE();
    ...
    //DMA 读取flash ，注意这里不能关中断，否则DMA没有中断导致操作失败
    {
